...
unsigned short decode_ethernet(const u_char *);
unsigned char decode_ip(const u_char *);
...
void caught_packet(u_char *user_args, const struct pcap_pkthdr *cap_header, const u_char *packet) {
	...
	if( decode_ethernet(packet) == 0x800) {
	
		if( decode_ip(packet+ETHER_HDR_LEN) == 6) {
...
unsigned short decode_ethernet(const u_char *header_start) {
	int i;
...
unsigned char decode_ip(const u_char *header_start) {
